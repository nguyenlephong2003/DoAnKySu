...
class BangBaoCaoTienDo {
    ...
    // Lấy danh sách tất cả các tiến độ của một công trình (KHÔNG JOIN)
    public function getAllProgressByProject($maCongTrinh) {
        $query = "SELECT * FROM BangBaoCaoTienDo WHERE MaCongTrinh = :maCongTrinh ORDER BY NgayBaoCao DESC";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':maCongTrinh', $maCongTrinh);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
