...
import { Table, Button, Input, Tag, Spin, message, Pagination, Modal, Form, DatePicker, InputNumber, Select, Upload, Descriptions, Card, Typography, Divider, Progress } from 'antd';
...
const QuanLyTienDo = () => {
        ...
        for (const baoCao of baoCaoResponse.data.data) {
          if (baoCao.MaCongTrinh && !congTrinhMap[baoCao.MaCongTrinh]) {
            try {
              ...
              console.error(`Error fetching construction info for ${baoCao.MaCongTrinh}:`, error);
            }
          }
        }
  ...
  const getFilteredData = () => {
    ...
    if (searchText) {
      filteredData = filteredData.filter(
        item => 
          ...
          (congTrinhList[item.MaCongTrinh]?.TenCongTrinh || '').toLowerCase().includes(searchText.toLowerCase())
      );
    }
    ...
    if (selectedConstruction !== 'all') {
      filteredData = filteredData.filter(item => item.MaCongTrinh === selectedConstruction);
    }
  ...
  const handleCreateBaoCao = async (values) => {
    try {
      ...
      const baoCaoData = {
        ...
        TrangThai: values.TrangThai || 'Chưa hoàn thành',
        TiLeHoanThanh: values.TiLeHoanThanh,
        HinhAnhTienDo: values.HinhAnhTienDo,
  ...
  const handleFileChange = async (info) => {
    if (info.file.status === 'done') {
      try {
        const fileUrl = await uploadFileAndGetURL(info.file.originFileObj);
        form.setFieldsValue({ HinhAnhTienDo: fileUrl });
  ...
  const handleEdit = (record) => {
    ...
    editForm.setFieldsValue({
      ...
      ThoiGianHoanThanhThucTe: record.ThoiGianHoanThanhThucTe ? dayjs(record.ThoiGianHoanThanhThucTe) : null
    });
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          const baoCaoToDelete = baoCaolist.find(bc => bc.MaTienDo === maTienDo);
          if (!baoCaoToDelete) {
          ...
          if (response.data.message === "Xóa báo cáo tiến độ thành công.") {
            ...
            message.error(response.data.message || 'Xóa báo cáo tiến độ thất bại');
          }
        } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="bg-white rounded-lg shadow-md p-6">
        <h1 className="text-3xl font-bold text-center mb-6">Quản lý báo cáo tiến độ</h1>
      ...
      >
        ...
        >
          ...
          <Form.Item
            ...
            rules={[{ required: true, message: 'Vui lòng nhập tỷ lệ hoàn thành' }]}
          >
            <InputNumber
              style={{ width: '100%' }}
              min={0}
          ...
          <Descriptions bordered column={2}>
            ...
            <Descriptions.Item label="Thời gian hoàn thành thực tế">
              {selectedBaoCao.ThoiGianHoanThanhThucTe ? 
                new Date(selectedBaoCao.ThoiGianHoanThanhThucTe).toLocaleDateString('vi-VN') : 
                'Chưa có'}
            </Descriptions.Item>
            ...
            <Descriptions.Item label="Trạng thái" span={2}>
              <Tag color={statusColors[selectedBaoCao.TrangThai] || 'default'}>
            ...
            {selectedBaoCao.HinhAnhTienDo && (
              <Descriptions.Item label="Hình ảnh tiến độ" span={2}>
                <img 
                  src={selectedBaoCao.HinhAnhTienDo} 
                  alt="Tiến độ công trình" 
